...
pub fn start_server(config: &Config, addr: crate::net::SocketAddr) -> Result<()> {
    ...
    let res = (|| -> io::Result<(crate::net::SocketAddr, Box<dyn FnOnce() -> io::Result<()>>)> {
        match &addr {
            crate::net::SocketAddr::Net(addr) => {
                let l = runtime.block_on(TcpListener::bind(addr))?;
                let srv = SccacheServer::<_, ProcessCommandCreator>::with_listener(l, runtime, client, dist_client, storage);
                let addr = srv.local_addr();
                Ok((addr, Box::new(move || srv.run(future::pending::<()>()))))
            }
            crate::net::SocketAddr::Unix(addr) => {
            ...
            crate::net::SocketAddr::Abstract(addr) => {
                let addr = std::os::unix::net::SocketAddr::from_abstract_name(addr)?;
...
pub struct SccacheServer<A: Acceptor, C: CommandCreatorSync> {
    runtime: Runtime,
    listener: A,
...
impl<C: CommandCreatorSync> SccacheServer<TcpListener, C> {
    pub fn new(
        port: u16,
        runtime: Runtime,
        client: Client,
    ...
    ) -> Result<SccacheServer<TcpListener, C>> {
        let addr = SocketAddrV4::new(Ipv4Addr::new(127, 0, 0, 1), port);
        let listener = runtime.block_on(TcpListener::bind(&std::net::SocketAddr::V4(addr)))?;
...
impl<A: Acceptor, C: CommandCreatorSync> SccacheServer<A, C> {
    ...
    /// Returns the address that this server is bound to
    #[allow(dead_code)]
