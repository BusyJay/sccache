...
fn get_addr() -> crate::net::SocketAddr {
    if let Some(port_str) = env::var("SCCACHE_SERVER_PORT").ok() {
        let port: Option<u16> = match port_str.parse() {
            Ok(p) => Some(p),
            Err(_) if cfg!(unix) => {
                warn!("unable to parse {port_str} as port, fallback to unix socket instead");
                None
            },
            Err(_) => {
                warn!("unable to parse {port_str} as port, fallback to default port instead");
                Some(DEFAULT_PORT)
            },
        };
        if let Some(port) = port {
            return crate::net::SocketAddr::Net(std::net::SocketAddr::new(
                std::net::IpAddr::V4(std::net::Ipv4Addr::new(127, 0, 0, 1)),
                port,
            ));
        }
    }
    let sock = env::var("SCCACHE_SERVER_SOCKET").unwrap_or_else(|_| "/tmp/sccache.sock".to_string());
    crate::net::SocketAddr::parse(&sock)
}
